...
import type { Action } from "@/lib/cadence/types";
...
function laneGeometry(h: number) {
  const scriptY = RULER_H + FLAG_H + 6;
  const scriptH = Math.max(56, h - scriptY - 8);
  return { scriptY, scriptH };
}
...
export function Timeline({ height = 140 }: TimelineProps) {
  ...
  const { scriptY, scriptH } = laneGeometry(size.h);
  ...
  return (
    <div ref={wrapRef} style={{ height: `${height}px` }} className="relative w-full bg-bg">
      <svg
        width={size.w}
        height={size.h}
        ...
        role="application"
        aria-label="Funscript editor timeline"
      >
        ...
        {/* A–B Region Visual Boundary */}
        {(abRegion.startMs !== null || abRegion.endMs !== null) && (
          <g>
            {/* Shaded Region between A and B if both exist */}
            {abRegion.startMs !== null && abRegion.endMs !== null && (() => {
              const regStart = Math.min(abRegion.startMs, abRegion.endMs);
              const regEnd = Math.max(abRegion.startMs, abRegion.endMs);
              const rx1 = Math.max(PAD_L, xAt(regStart, start, span, innerW));
              const rx2 = Math.min(PAD_L + innerW, xAt(regEnd, start, span, innerW));
              ...
              return (
                <g>
                  <rect
                    ...
                    y={RULER_H + 2}
                    width={rw}
                    height={size.h - RULER_H - 10}
                    fill={abRegion.confirmed ? "rgba(56, 189, 248, 0.10)" : "rgba(245, 158, 11, 0.08)"}
                    stroke={abRegion.confirmed ? "#38bdf8" : "#f59e0b"}
                    strokeWidth="1"
                    strokeDasharray="4 3"
                    rx="3"
                  />
                  <text
                    x={rx1 + 6}
                    y={RULER_H + 16}
                    fill={abRegion.confirmed ? "#38bdf8" : "#f59e0b"}
                    fontSize="10"
                    fontWeight={600}
                    fontFamily="Outfit, sans-serif"
                  >
                    A–B Region {abRegion.confirmed ? "(Confirmed)" : "(Draft)"}
                  </text>
                </g>
              );
            })()}

            {/* Region A Marker Line */}
            {abRegion.startMs !== null && (() => {
              const ax = xAt(abRegion.startMs, start, span, innerW);
              if (ax < PAD_L - 10 || ax > PAD_L + innerW + 10) return null;
              return (
                <g>
                  <line
                    x1={ax}
            ...
            {/* Region B Marker Line */}
            {abRegion.endMs !== null && (() => {
              const bx = xAt(abRegion.endMs, start, span, innerW);
              if (bx < PAD_L - 10 || bx > PAD_L + innerW + 10) return null;
              return (
                <g>
                  <line
                    x1={bx}
        ...
        <g clipPath="url(#cadence-script)">
          ...
          {/* Genuine Empty State indicator when no fake/demo actions are loaded */}
          {actions.length === 0 && (
            <text
              x={PAD_L + innerW / 2}
              y={scriptY + scriptH / 2 + 4}
              textAnchor="middle"
              fill="var(--color-subtle)"
              fontSize="12"
              fontFamily="Outfit, sans-serif"
              opacity={0.65}
            >
              Script editor empty · Click in lane to insert action nodes or import .funscript
            </text>
          )}
